...
$(function(){
    ...
    mapArray=[//0-可走,1-障礙,2-終點,3-敵人
        ...
        imgMain.onload=function(){
            ...
            //ctx.drawImage(imgMain,350,0,80,130,currentImgMain.x+200,currentImgMain.y,gridLength,gridLength);
        };
...
$(document).on("keydown",function(event){
    ...
    if(targetImg.x<=400&&targetImg.x>=0&&targetImg.y<=400&&targetImg.y>=0){
        ...
        targetBlock.x=-1;targetBlock.y=-1;
    }//清空主角原本所在的位置
    ...
    if(targetBlock.x!=-1&&targetBlock.y!=-1)
    {
        switch(mapArray[targetBlock.x][targetBlock.y]){
            ...
            case 2://終點(可移動)
            $("#talkBox").text("抵達終點");
            currentImgMain.x=targetImg.x;currentImgMain.y=targetImg.y;
            break;
            case 3://敵人(不可移動)
            $("#talkBox").text("扁你喔");break;
        }
    }
    else{
        $("#talkBox").text("邊界");
